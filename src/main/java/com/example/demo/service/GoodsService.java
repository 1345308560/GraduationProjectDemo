...
public class GoodsService {
    ...
    public List<Map<String,Object>> findUserAllGoods(Integer userId, Integer pagenum, Integer pagesize) {
        // 获取商品的总数
        int total = countUserGoods(userId);
        // 计算总页数
        int totalPage = total/pagesize + 1;
        // 如果传入参数超出页数范围，则返回空
        if(pagenum > totalPage || pagenum < 1){
    ...
    public List<Map<String,Object>> findUserCertainGoods(Integer userId, Integer pagenum, Integer pagesize, String kind, String query) {
        // 获取商品的总数
        int total = getUserCertainPage(userId,kind,query);
        // 计算总页数
        int totalPage = total/pagesize + 1;
        // 如果传入参数超出页数范围，则返回空
        if(pagenum > totalPage || pagenum < 1){
