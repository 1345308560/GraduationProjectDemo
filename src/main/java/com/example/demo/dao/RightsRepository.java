...
public interface RightsRepository extends CrudRepository<Rights, String> {
    ...
    Optional<Rights> findByUid(Integer uid);

    //查询全部权限，返回user和user_rights
    @Query(value="select u.*,r.rights_buy,r.rights_sell from user u join rights r on u.id=r.uid where u.display = 0 limit ?1, ?2" ,nativeQuery=true)
